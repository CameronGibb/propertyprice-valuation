    ...
    ifelse(file.exists('data/by_outcde_yr_typ.rds'),
           ...
           by_outcde_yr_typ <- full_data[, 
                                         .(.N, 
                                           ...
                                           log10_avg_price = log10(mean(price)),
...
```
CG - adding in some data-exp
```{r}
# Creating a subset of terrace houses in one single tax year (to avoid spread caused by HP inflation).
# Terrace is the most populous group across the full dataset range. 2015 is the most populous year.
# Could also assume that terraced houses are less likely to have "unique" features (such as large footprint) when compared with detached that might result in price outliers. 
# Still, terrace captures some central London residences, with sales prices well in £10 millions which is where the long tail comes from.
terrace <- full_data %>% 
  filter(prop_typ == "T" & tax_year %in% c(2017, 2018, 2019)) %>% 
   mutate(log_price = log(price))

library(ggplot2)
# View of distribution- this follows a fairly lognormal shape
ggplot(terrace, aes(x=price)) + geom_histogram(binwidth=20000) + xlim(0, 1E6)
...
ggplot(terrace, aes(x=log10(price))) + geom_histogram(binwidth=0.1)

# For a single postcode outcode (will use the most populous) - CV6 (Coventry)
# Appears to be a split here somewhere around £170k
terrace_CV6 <- terrace %>%  filter(outcde == "CV6")
ggplot(terrace_CV6, aes(x=price)) + geom_histogram(binwidth=10000)
ggplot(terrace_CV6, aes(x=log10(price))) + geom_histogram(binwidth=0.05)
```
...
check_logsigma <- function(outcode, p_type, t_year){
  x <- filter(full_data, outcde == outcode & prop_typ == p_type & tax_year == t_year)
  sbst <- filter(by_outcde_yr_typ_, outcde == outcode & prop_typ == p_type & tax_year == t_year)
  sigma_upper <- sbst$avg_logsigma_pos
  sigma_lower <- sbst$avg_logsigma_neg
  x_ <- filter(x, price <= sigma_upper & price >= sigma_lower)
  print(
    paste0("Number of transactions within logsigma confidence interval: ", as.character(100 * nrow(x_) / nrow(x)))
    )
}

check_logsigma("SW11", "F", 2017)
